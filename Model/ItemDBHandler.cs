...
namespace SkyrimCraftingTool.Model
{
    public class ItemDBHandler
    {
        ...
        private void LoadCache()
        {
            ...
            _cobjCache = LoadCOBJ();
            _cobjCondtionsCache = LoadCOBJConditions();
        ...
        public void PutIntoDataBank(List<PluginInfo> allgamePathfromDB)
        {
            ...
            using var insertCOBJCondition = PrepareInsertCOBJ_Conditions(connection);
            ...
            foreach (var plugin in allgamePathfromDB)
            {
                ...
                foreach (var fullPath in plugin.FullPaths)
                {
                    ...
                    foreach (var cobj in mod.ConstructibleObjects.Records)
                    {
                        ...
                        {
                            // ConditionFloat vergleicht gegen einen festen Float-Wert,
                            // ConditionGlobal vergleicht gegen einen Global. Beide werden
                            // von ESP/ESL/ESM Dateien produziert, daher müssen wir beide lesen.
                            // Beim Lesen aus Plugin-Dateien liefert Mutagen Overlay-Typen
                            // (z.B. ConditionFloatBinaryOverlay), die nur die Getter-Interfaces
                            // implementieren, nicht die konkreten Klassen. Daher gegen die
                            // Interfaces matchen statt gegen ConditionFloat/ConditionGlobal.
                            ...
                            insertCOBJCondition.Parameters["@cobjKey"].Value = key;
                            insertCOBJCondition.Parameters["@extra"].Value = ""; // aktuell ungenutzt
                            insertCOBJCondition.Parameters["@runOn"].Value = data.RunOnType.ToString();
                            ...
                            if (data is IHasPerkConditionDataGetter perkData)
                            {
                                ...
                                insertCOBJCondition.Parameters["@type"].Value = "HasPerk";
                                insertCOBJCondition.Parameters["@target"].Value = target;
                                insertCOBJCondition.Parameters["@value"].Value = comparisonValue;

                                insertCOBJCondition.ExecuteNonQuery();
                                continue;
                            }
                            ...
                            if (data is IGetIsSexConditionDataGetter sexData)
                            {
                                ...
                                insertCOBJCondition.Parameters["@type"].Value = "GetIsSex";
                                insertCOBJCondition.Parameters["@target"].Value = target;
                                insertCOBJCondition.Parameters["@value"].Value = comparisonValue; // sollte 1 sein
                            ...
                            if (data is IGetActorValueConditionDataGetter avData)
                            {
                                ...
                                string value = comparisonValue;

                                insertCOBJCondition.Parameters["@type"].Value = "GetActorValue";
                                insertCOBJCondition.Parameters["@target"].Value = target;
                                insertCOBJCondition.Parameters["@value"].Value = value;
                            ...
                            if (data is IGetLevelConditionDataGetter)
                            {
                                ...
                                insertCOBJCondition.Parameters["@type"].Value = "GetLevel";
                                insertCOBJCondition.Parameters["@target"].Value = ""; // kein Target
                                insertCOBJCondition.Parameters["@value"].Value = value;
                            ...
                            if (data is IGetStageDoneConditionDataGetter stageData)
                            {
                                ...
                                insertCOBJCondition.Parameters["@type"].Value = "GetStageDone";
                                insertCOBJCondition.Parameters["@target"].Value = target;
                                insertCOBJCondition.Parameters["@value"].Value = stageData.Stage.ToString();
        ...
        private void ResetTables(SqliteConnection connection)
        {
            ...
            @"
                ...
                DROP TABLE IF EXISTS COBJ_Conditions;
        ...
        private SqliteCommand PrepareInsertCOBJ(SqliteConnection connection) // Maybe here overwrite error
        {
            var cmd = new SqliteCommand(
        ...
        private SqliteCommand PrepareInsertCOBJ_Conditions(SqliteConnection connection)
        {
            var cmd = new SqliteCommand(
                "INSERT OR REPLACE INTO COBJ_Conditions (COBJKey, ConditionType, Target, Value, Extra, RunOn)" +
                "VALUES (@cobjKey, @type, @target, @value, @extra, @runOn)",
                connection);

            cmd.Parameters.Add("@cobjKey", SqliteType.Text);
            cmd.Parameters.Add("@type", SqliteType.Text);
            cmd.Parameters.Add("@target", SqliteType.Text);
            cmd.Parameters.Add("@value", SqliteType.Text);
            cmd.Parameters.Add("@extra", SqliteType.Text);
            cmd.Parameters.Add("@runOn", SqliteType.Text);
        ...
        public void SaveCOBJConditions(string cobjKey, List<COBJConditionRecord> conditions)
        {
            ...
            if (conditions != null)
            {
                using var insertCmd = PrepareInsertCOBJ_Conditions(connection);
