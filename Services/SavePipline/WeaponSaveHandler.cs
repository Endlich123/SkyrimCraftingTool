        ...
        public Task HandleAsync(SaveRequest r)
        {
            ...
            switch (r.FieldName)
            {
                ...
                case nameof(ItemNodeVM.SelectedKeywordKeys):
                    // SelectedKeywordKeys ist eine List<string> der selektierten Keyword-Keys
                    _itemService.UpdateWeaponKeywords(item.Key, item.SelectedKeywordKeys);
                    _cache.UpdateWeaponKeywords(item.Key, item.SelectedKeywordKeys);
                    break;
