...
namespace SkyrimCraftingTool.Services
{
    public class CacheManager : ICacheManager
    {
        ...
        public CacheSnapshot BuildCachesFromDB(List<PluginInfo> activePlugins)
        {
            ...
            // Warm the shared ItemDBHandler cache once, up front, on this thread. Otherwise every
            // worker below would hit the load lock at once and all but one would just block on it.
            _itemService.EnsureCacheLoaded();
            ...
            Parallel.ForEach(activePlugins, plugin =>
            {
                ...
                {
                    recipe.Conditions = _itemService.GetCOBJConditions(recipe.Key);
                    lock (recipeLocal)
                    {
            ...
            snapshot.Perks = _formIdService.SearchByType("Perk")
                ...
                .Select(g => g.First())
                .ToList();
            snapshot.Quests = _formIdService.SearchByType("Quest")
                .GroupBy(q => q.Key)
                .Select(g => g.First())
                .ToList();
        ...
        public void UpdateRecipeConditions(string cobjKey, List<COBJConditionRecord> conditions)
        {
            if (string.IsNullOrEmpty(cobjKey)) return;

            lock (_recipeLock)
            {
                foreach (var list in _snapshot.RecipesByCreatedItem.Values)
                {
                    var rec = list.FirstOrDefault(r => r.Key == cobjKey);
                    if (rec != null)
                        rec.Conditions = conditions ?? new List<COBJConditionRecord>();
                }
            }
        }

        // Drops a recipe entirely from the snapshot - counterpart to ItemDBHandler.DeleteCOBJ, used
        // when the user resets a recipe they created from scratch (see MainContentVM.
        // ResetCraftingRecipeEdits/ResetTemperRecipeEdits) instead of one that already existed in the
        // plugin. Without this the deleted row would still linger here until the next full rescan.
        public void RemoveRecipe(string cobjKey)
        {
            if (string.IsNullOrEmpty(cobjKey)) return;
            ...
            {
                foreach (var list in _snapshot.RecipesByCreatedItem.Values)
                    list.RemoveAll(r => r.Key == cobjKey);
            }
        }
